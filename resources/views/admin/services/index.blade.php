<x-sidebar-layout>
    ...
    <div x-data="{ 
            ...
            showEditModal: false, 
            editData: { id: '', nama_paket: '', deskripsi: '', estimasi_menit: '', harga_jasa: '' },
            openEditModal(item) {
                this.editData = item;
            ...
            <div class="bg-white rounded-[24px] border border-slate-200 shadow-sm overflow-hidden flex flex-col relative group hover:border-red-300 transition-colors">
                ...
                <div class="bg-slate-50 px-6 py-4 flex justify-between items-center border-t border-slate-100">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs font-bold text-slate-500 uppercase tracking-widest">Servis & Jasa</span>
                    </div>
        ...
        <div x-cloak x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div @click.away="showEditModal = false" class="bg-white rounded-[24px] shadow-2xl w-full max-w-lg overflow-hidden transform transition-all">
                ...
                <form :action="'/admin/services/' + editData.id" method="POST" class="p-6 space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Nama Paket</label>
