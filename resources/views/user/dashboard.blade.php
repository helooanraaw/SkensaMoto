<x-sidebar-layout>
    ...
    <div x-data="dashboardManager" x-effect="document.body.style.overflow = (showModal || showEditModal) ? 'hidden' : ''" class="space-y-8">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div class="xl:col-span-2 space-y-8">
                ...
                <!-- Booking Form (Now below Garasi) -->
                <div class="bg-white rounded-[24px] border border-red-200 shadow-xl shadow-red-100/50 overflow-hidden">
                    <div class="bg-red-600 p-6 text-white text-center sm:text-left flex flex-col sm:flex-row justify-between items-center gap-2">
                        <div>
                            <h3 class="text-xl font-bold mb-1">Buat Booking Servis</h3>
                            <p class="text-red-100 text-sm font-thin">Isi form di bawah untuk memesan slot antrean</p>
                        </div>
                    </div>
                    <div class="p-6">
                        ...
                        <form action="{{ route('user.booking.store') }}" method="POST" class="space-y-5" 
                            ...
                            <!-- Responsive 2 columns for Motor and Tanggal -->
                                    ...
                                    <select name="tanggal" required class="w-full border-slate-200 rounded-xl px-4 py-3 bg-slate-50 text-blue-950 font-bold focus:ring-red-500">
                                        @foreach($activeSchedules as $s)
                                            @if($s->kapasitas_menit - $s->terpakai_menit > 0)
                                                <option value="{{ $s->tanggal }}" {{ (request('tanggal') == $s->tanggal || request('date') == $s->tanggal) ? 'selected' : '' }}>{{ \Carbon\Carbon::parse($s->tanggal)->format('d M Y') }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Keluhan / Catatan</label>
                                <textarea name="keluhan" rows="3" placeholder="Jelaskan kendala motor Anda secara rinci..." class="w-full border-slate-200 rounded-xl px-4 py-3 bg-slate-50 text-blue-950 font-bold focus:ring-red-500 focus:border-red-500"></textarea>
                            </div>
            ...
            <!-- Right Column: Riwayat & Antrean -->
            <div class="xl:col-span-1">
                <div class="bg-white rounded-[24px] border border-slate-200 shadow-sm overflow-hidden flex flex-col sticky top-24">
                    ...
                    <div class="p-6 space-y-4 max-h-[calc(100vh-250px)] overflow-y-auto custom-scrollbar">
                        @forelse($bookings->take(5) as $b)
                            <div class="border border-slate-200 rounded-2xl p-5 bg-white hover:border-red-300 transition shadow-sm group">
                                <div class="flex flex-col sm:flex-row xl:flex-col xxl:flex-row justify-between items-start gap-4 mb-3">
                                    ...
                                    <div class="shrink-0">
                                        ...
                                        @elseif($b->status === 'completed')
                                            @if(($b->payment_status ?? 'unpaid') === 'paid')
                                                <span class="px-3 py-1 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap">Lunas</span>
                                            @else
                                                <span class="px-3 py-1 bg-rose-50 text-rose-600 border border-rose-100 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap">Belum Bayar</span>
                                            @endif
                                        @else
                                ...
                                <div class="sm:pl-[52px] xl:pl-0 xxl:pl-[52px]">
                        ...
                        <div class="p-6 overflow-y-auto flex-1 space-y-6">
                            ...
                            <div class="rounded-[16px] p-4 text-center font-bold text-sm uppercase tracking-widest border"
                                 :class="{
                                     ...
                                     'bg-emerald-50 text-emerald-700 border-emerald-100': activeBooking?.status === 'completed' && activeBooking?.payment_status === 'paid',
    ...
    <template x-teleport="body">
        <div x-cloak x-show="showEditModal" class="fixed inset-0 z-[10000]">
            <!-- Backdrop Blur -->
            <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-md"></div>
            ...
            <div class="fixed inset-0 overflow-y-auto flex items-center justify-center p-4">
                <template x-if="showEditModal">
                    <div @click.away="showEditModal = false" class="bg-white rounded-[24px] shadow-2xl w-full max-w-md overflow-hidden flex flex-col my-8 relative">
                        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
                            <div>
